    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        try {
            ...
            if (ex instanceof ExpiredJwtException) {
                response.getWriter().write(new ObjectMapper().writeValueAsString(new ErrorResponse("El token ha expirado", 498)));
                response.setStatus(498);
            } else {
                response.getWriter().write(new ObjectMapper().writeValueAsString(new ErrorResponse(ex.getMessage(), HttpStatus.UNAUTHORIZED.value())));
